...
kms_show_data_connect_video (KmsShowData * self, GstElement * agnosticbin)
{
  ...
  //gst_element_link (agnosticbin, self->priv->imgelm);
...
kms_show_data_connect_data (KmsShowData * self, GstElement * tee)
{
  ...
  g_signal_connect (identity, "handoff", G_CALLBACK (new_datax), self->priv->imgelm);
  //g_signal_connect (identity, "handoff", G_CALLBACK (new_datax), self->priv->imglem);
...
kms_show_data_init (KmsShowData *self)
{
  ...
  self->priv->imgelm = gst_element_factory_make ("graphsmetadata", NULL);
